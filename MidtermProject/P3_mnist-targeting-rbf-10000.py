"""
file to handselect targeting of rbf parameters for further tuning and analysis
"""
...
INITIAL_FIT_TIMES = "initial-fit-times_rbf-10000.csv" # for the initial fit times of everything in the pipeline
TEST_AND_TRAIN_ACC = "test-train-acc_rbf-10000.csv" # for the accuracy of final SVC on test, then train data
FINAL_TIMES = "final-times_rbf-10000.csv" # for the final times, that includes final transforms of test data
...
combos = [
  ...
  (100, 0.01),
  (1000, 0.01),
  (0.01, 10), # wildcard ones
  (0.1, 10), # wildcard ones
  (1, 10), # wildcard ones
]
...
for dim_reducer_name in dim_reducers:
  ...
  for (c, g) in combos: # to run through hand tuned parameters
 ...
 # write this info to appropriate CSV 
  ...
  df.to_csv(os.path.join(MNIST_DATA_PATH, INITIAL_FIT_TIMES), index=False, mode='a', header=False)

  df = pd.DataFrame(csv_test_train_acc_line)
  df.to_csv(os.path.join(MNIST_DATA_PATH, TEST_AND_TRAIN_ACC), index=False, mode='a', header=False)

  df = pd.DataFrame(csv_final_times_line)
  df.to_csv(os.path.join(MNIST_DATA_PATH, FINAL_TIMES), index=False, mode='a', header=False)
